// Update the data source to a relative path
const DATA_URL = "excel.json"
// ...existing code...
...
function populate_projects(proj_list) {
    for (var i = 0; i < proj_list.length; i++) {
        ...
        // Create a project object with the new structure
        var project = {
            "Station Id": proj_list[i]["Station Id"],
            "Station Name": proj_list[i]["Station Name"],
            "City": proj_list[i]["Centre (City)"],
            "Domain": proj_list[i]["Business Domain"],
            "Project Domain": proj_list[i]["Project Domain"],
            "Title": proj_list[i]["Title"],
            "Description": proj_list[i]["Description"],
            "Tags": proj_list[i]["Tags"]
        };
        station_proj_map.get(key).push(project);
    }
}

function load_data() {
    // Since this is a Chrome extension, we can use chrome.runtime.getURL
    // to get the absolute URL of the JSON file
    const jsonUrl = chrome.runtime.getURL(DATA_URL);
...
function export_exel() {
    ...
    XLSX.writeFile(workbook, "PS1_Preference.xlsx", { compression: true });
...
function fill_details(opt, proj_index) {
    ...
    if (station_proj_map.has(opt_name)) {
        ...
        subdomain_field.innerHTML = project["Project Domain"] || "N/A"
        desc_field.innerHTML = project["Description"] || "N/A"
    } else {
        resetFields();
    }
}
